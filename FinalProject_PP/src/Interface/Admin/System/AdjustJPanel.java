...
public class AdjustJPanel extends javax.swing.JPanel {
    ...
    AdjustJPanel(JPanel CommandContainer, EcoSystem system) {
        initComponents();
        this.CommandContainer = CommandContainer;
        this.system = system;
        system.setMoney(1000.0);
        lblMoney.setText(String.valueOf(system.getMoney()));
        populatecbx1();
        populatecbx2();
        cost();
    }
    public void cost(){
        ...
        Country country= (Country)cbxArea2.getSelectedItem();
        double distance;
        Country country1 = (Country)cbxArea1.getSelectedItem();
        double temp = Math.pow(country1.getLocation_x()-country.getLocation_x(), 2) + Math.pow(country1.getLocation_y()-country.getLocation_y(), 2);
    ...
    public void populateRequest() {
        ...
        dtm.setRowCount(0);
        Country cbCountry = (Country)cbxArea2.getSelectedItem();
        Object near=null;
        Double nearDistance = 100000000000.0;
        for (WorkRequest workRequest : cbCountry.getWorkQueue().getWorkRequestList()) {
            Object row[] = new Object[3];
            row[0] = workRequest;
            row[1] = workRequest.getItem().getQuantity();
            //nearest district
            for(Country copcountry : system.getCountryList()){
             double distance;
             double temp = Math.pow(copcountry.getLocation_x()-cbCountry.getLocation_x(), 2) + Math.pow(copcountry.getLocation_y()-cbCountry.getLocation_y(), 2);
             distance = Math.sqrt(temp);
             if(nearDistance >=distance&& distance!=0){
                for(Item copItem:copcountry.getItemDirectory().getItemList()){
    ...
    private void btnTransferMActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnTransferMActionPerformed
        ...
        if (selectedRow < 0) {
        ...
        } else if (Double.parseDouble(txtCost.getText())>system.getMoney()) {
            ...
            system.setMoney(system.getMoney()-Double.parseDouble(txtCost.getText()));
            lblMoney.setText(String.valueOf(system.getMoney()));
            populateMaterial1();
            populateMaterial2();
            populateRequest();
