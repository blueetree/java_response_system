...
import Interface.Salesperson.SalesmanMainJFrame;
...
public class MainJFrame extends javax.swing.JFrame {
    ...
    public MainJFrame() {
        ...
        initialize1st();
        ConfigABusiness.getConfig(ud,pd,marketList,supplierDirectory, marketOfferCatalog,spd);
      
        
        UserAccount uas = new UserAccount();
        uas = ud.addUserAccount();
        uas.setRole(Role.Procurement_Supplier);
        uas.setUserName("123");
        String input = "123";
    //   int password=input.hashCode();
      String encoded =DatatypeConverter.printBase64Binary(input.getBytes());
      System.out.println(encoded);
      uas.setPassWord(encoded);
        Supplier sp = supplierDirectory.getSupplierDirectory().get(0);
        uas.setSupplier(sp);
 ...
 {      
        ...
        ua.setUserName("Lora");
        String input = "123";
         UserAccount ua2=ud.addUserAccount();
        ua2.setPerson(person);
        ua2.setUserName("Salesman");
        String input2 = "123";
    //   int password=input.hashCode();
      String encoded =DatatypeConverter.printBase64Binary(input.getBytes());
      System.out.println(encoded);
       ...
       String encoded2 =DatatypeConverter.printBase64Binary(input2.getBytes());
      System.out.println(encoded2);

        ua2.setPassWord(encoded);
        ua2.setRole(Role.Sales_Person);  
        SalesPerson sm=salesPersonDirectory.addSalesPerson();
        sm.setUa(ua2);
        Market market=marketList.addMarket();
        market.setMarketName("market1");
        Customer customer=market.addCustomer();
        customer.setCustomerName("Leo");
        customer.setAddress("Westland Ave 15th");
    ...
    System.out.println("decoded"+decoded);
     ...
     if(passWord.equals(decoded))
     {
         ...
         if(account.getRole()==Role.System_admin)
                 {
                    AdminWorkAreaJPanel awajp = new AdminWorkAreaJPanel(userProcessContainer, ud, account,pd,supplierDirectory,marketOfferCatalog);
                    userProcessContainer.add("AdminWorkAreaJPanel",awajp);
                    CardLayout layout = (CardLayout)userProcessContainer.getLayout();
                    layout.next(userProcessContainer);
                 }
           if(account.getRole()==Role.Procurement_Supplier)
                 {
                     Supplier s = account.getSupplier();
                    AdminProductAreaJPanel awajp = new AdminProductAreaJPanel(userProcessContainer, account,s);
                    userProcessContainer.add(" AdminPersonAreaJPanel ",awajp);
                    CardLayout layout = (CardLayout)userProcessContainer.getLayout();
                    layout.next(userProcessContainer);
                 }
           if(account.getRole()==Role.Sales_Person)
                 {
                   SalesPerson pointedSm=salesPersonDirectory.searchSalesman(account);
                   SalesmanMainJFrame awajp = new SalesmanMainJFrame(salesPersonDirectory,marketList,supplierDirectory,userProcessContainer, pointedSm,account,marketOfferCatalog);
                    userProcessContainer.add(" AdminPersonAreaJPanel ",awajp);
                    CardLayout layout = (CardLayout)userProcessContainer.getLayout();
                    layout.next(userProcessContainer);
                 }
