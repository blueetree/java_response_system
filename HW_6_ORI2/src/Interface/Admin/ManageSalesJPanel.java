...
public class ManageSalesJPanel extends javax.swing.JPanel {
    ...
    public ManageSalesJPanel(JPanel userProcessContainer,SalesPersonDirectory spd,SupplierDirectory sd,UserAccount account,PersonDirectory pd,MarketOfferCatalog moc) {
        initComponents();
        this.spd=spd;
        this.sd = sd;
        ...
        for(SalesPerson salesPerson :spd.getSalesPersonDirectory()){
            for(Order order : salesPerson.getOrderCatalog()){
                ...
                double revenue = (orderItem.getActualPrice() -value)*count;
                double missed = (orderItem.getActualPrice()-target)*count;
                totalsRevenue = totalsRevenue + revenue;
       ...
       txtMissed.setText(String.valueOf(totalMissed));
        ...
        //make compensation, and then penalty and rewards based on the sales person negotiating decision
  ...
  public void populateRevenueTable(){
      //revenue by market
        ...
        for (MarketOffer mo :moc.getMarketOfferCatalog()){
           double marketRevenue = 0;
           double missedRevenue = 0;
           Object row[] = new Object[4];
           row[0] = mo.getMarket();
           row[1] = mo.getMarket().getMarketName();
        for (SalesPerson salesPerson :spd.getSalesPersonDirectory()) {
            for(Order order : salesPerson.getOrderCatalog()){
                for(OrderItem orderItem : order.getOrderItemList()){
           if(orderItem.getMarketOffer().getMarket().getMarketID().equals(mo.getMarket().getMarketID())){
               double value = orderItem.getMarketOffer().getProduct().getProductValue();
               double target = orderItem.getMarketOffer().getTargetPrice();
               int count = orderItem.getQuantity();
               double revenue = (orderItem.getActualPrice() -value)*count;
               double missed = (orderItem.getActualPrice()-target)*count;
               marketRevenue = marketRevenue + revenue;
               marketRevenue = marketRevenue + missed;
              } 
            }
           }
          }
    ...
    public void populateProductTable(){
       ...
       for (SalesPerson salesPerson :spd.getSalesPersonDirectory()) {
            for(Order order : salesPerson.getOrderCatalog()){
                for(OrderItem orderItem : order.getOrderItemList()){
                if(orderItem.getActualPrice()<orderItem.getMarketOffer().getTargetPrice()){
                    belowProductID.add(orderItem.getMarketOffer().getProduct().getProductID());
        ...
        for (SalesPerson salesPerson :spd.getSalesPersonDirectory()) {
            for(Order order : salesPerson.getOrderCatalog()){
                ...
                for(String belowProduct : belowProductID){
                    if(orderItem.getMarketOffer().getProduct().getProductID().equals(belowProduct)){
                       equal = true; 
